  ...
  plugins: [react()],
   server: {
    headers:{
      ...
      'Content-Security-Policy': "default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval' http://localhost:5173; style-src 'self' 'unsafe-inline'; img-src 'self' data: http://localhost:5173; font-src 'self'; connect-src 'self' ws://localhost:5173 http://144.22.182.60:8888 https://viacep.com.br;"
    }
  }
})
